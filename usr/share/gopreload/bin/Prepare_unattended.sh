...
if [ ! -d "$INSTALLDIR" ] ; then
	echo
	echo "Couldn't find $INSTALLDIR, exiting now."
	echo
	exit 1
fi
...
if [ ! -d "$INSTALLDIR/enabled" ]; then
	echo
	echo "$INSTALLDIR/enabled missing!"
...
if [ ! -w "$INSTALLDIR/enabled" ]; then
	echo
	echo "You have not write permissions to $INSTALLDIR/enabled,"
	...
	echo "to make it writable by you."
	echo
	exit 1
fi
...
if [ ! -w "$INSTALLDIR/disabled" ]; then
	echo
	echo "You have not write permissions to $INSTALLDIR/disabled,"
	echo "because gopreload defaults that way for regular users."
...
while true; do
	...
	NUMWIN=$NUMWIN2
	kill -0 $PIDOFSTRACE &>/dev/null || break #exit as soon as strace exits
done
